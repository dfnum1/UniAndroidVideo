  ...
  extends ContentProvider
{
 ...
 // private static final String NO_FILE = "N";
  ...
  private boolean initIfNecessary()
  {
    if (!this.mInit)
    {
      ...
      if (pi.metaData != null)
      {
        ...
        String mainFileName = pi.metaData.getString("mainFilename", "N");
        if ("N" != mainFileName)
        {
          String patchFileName = pi.metaData.getString("patchFilename", "N");
          if ("N" != patchFileName) {
  ...
  public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs, String sortOrder)
  {
    ...
    else
    {
      ...
      for (int i = 0; i < len; i++) {
        if (projection[i].equals("_id")) {
          intProjection[i] = 0;
        } else if (projection[i].equals("ZPFN")) {
          intProjection[i] = 1;
        } else if (projection[i].equals("ZFIL")) {
          intProjection[i] = 2;
        } else if (projection[i].equals("ZMOD")) {
          intProjection[i] = 3;
        } else if (projection[i].equals("ZCRC")) {
          intProjection[i] = 4;
        } else if (projection[i].equals("ZCOL")) {
          intProjection[i] = 5;
        } else if (projection[i].equals("ZUNL")) {
          intProjection[i] = 6;
        } else if (projection[i].equals("ZTYP")) {
          intProjection[i] = 7;
        } else {
    ...
    int j = (arrayOfZipEntryRO1 = zipEntries).length;
    for (int i = 0; i < j; i++)
    {
      ZipResourceFile.ZipEntryRO zer = arrayOfZipEntryRO1[i];
      MatrixCursor.RowBuilder rb = mc.newRow();
      for (int k = 0; k < len; k++) {
        switch (intProjection[k])
        {
        case 0: 
          rb.add(Integer.valueOf(k));
          break;
        case 1: 
